...
const menuItems = [
  { path: "/", label: "首页", icon: House },
  {
    ...
    icon: Document,
    children: [
      { path: "/colleges/schemes", label: "Schema", icon: Collection },
      { path: "/colleges/imports", label: "导入数据", icon: Collection },
      { path: "/colleges", label: "添加集合", icon: Plus },
    ],
  },
...
<template>
  <div
    ...
    <aside
      class="flex flex-col w-64 h-full bg-white/85 backdrop-blur-xl border-r border-slate-200/70 shadow-sm">
      <div
        class="px-5 py-5 flex items-center gap-3 border-b border-slate-200/70">
        <div
      ...
      <div class="p-4 border-t border-slate-200/70 bg-white/70">
        <div
          ...
          <div
            class="h-9 w-9 rounded-full bg-slate-100 grid place-items-center text-slate-600 text-sm font-semibold">
          ...
          <div class="min-w-0 flex-1">
            ...
            <div class="text-[11px] text-slate-500 truncate">
              {{ (user as any)?.email || "Not signed in" }}
            </div>
          </div>
          <ElButton size="small" type="danger" plain @click="logout"
            >退出</ElButton
          >
        </div>
      </div>
