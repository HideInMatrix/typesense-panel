    ...
    <div class="space-y-4">
      ...
      <div
        ...
        <ElFormItem label="源字段 (from)">
          <div class="space-y-2">
            <div
              v-for="(_field, index) in embedFromFields"
              :key="index"
              class="flex items-center gap-2">
              <ElInput
                v-model="embedFromFields[index]"
                placeholder="输入字段名"
                class="flex-1" />
              <ElButton
                @click="emit('removeEmbedFromField', index)"
                type="danger"
                size="small"
                plain>
                <ElIcon>
        ...
        <ElFormItem label="模型配置 (model_config)">
          <div class="space-y-3">
            <!-- 现有配置项 -->
            <div
              v-for="(item, index) in modelConfigItems"
              :key="index"
              ...
              <ElButton
                @click="emit('removeModelConfigItem', index)"
                type="danger"
                size="small"
                plain>
                <ElIcon>
            ...
            <!-- 添加新配置项 -->
            <div
              class="flex items-center gap-2 p-3 border-2 border-dashed border-slate-300 rounded-lg">
              <div class="flex-1 grid grid-cols-2 gap-2">
                <ElInput v-model="newModelConfigKey" placeholder="配置键" />
                <ElInput v-model="newModelConfigValue" placeholder="配置值" />
              </div>
              <ElButton
                @click="emit('addModelConfigItem')"
                type="primary"
                size="small">
                <ElIcon>
                  <Plus />
