...
export const useFieldEditor = () => {
  ...
  const fieldTypes = [
    {
      value: "string",
      label: "字符串 (string)",
      description: "文本字段，支持全文搜索",
    },
    {
      value: "int32",
      label: "32位整数 (int32)",
      description: "整数类型，支持排序和分面",
    },
    {
      value: "int64",
      label: "64位整数 (int64)",
      description: "长整数类型，支持排序和分面",
    },
    {
      value: "float",
      label: "浮点数 (float)",
      description: "浮点数类型，支持排序和分面",
    },
    {
      value: "bool",
      label: "布尔值 (bool)",
      description: "布尔类型，支持分面",
    },
    {
      value: "string[]",
      label: "字符串数组 (string[])",
      description: "字符串数组，支持多值",
    },
    {
      value: "int32[]",
      label: "整数数组 (int32[])",
      description: "整数数组，支持多值",
    },
    {
      value: "int64[]",
      label: "长整数数组 (int64[])",
      description: "长整数数组，支持多值",
    },
    {
      value: "float[]",
      label: "浮点数组 (float[])",
      description: "浮点数组，支持多值",
    },
    {
      value: "bool[]",
      label: "布尔数组 (bool[])",
      description: "布尔数组，支持多值",
    },
    {
      value: "geopoint",
      label: "地理坐标 (geopoint)",
      description: "地理坐标点，支持地理位置搜索",
    },
    ...
    },
    {
      value: "embed",
      label: "嵌入向量 (embed)",
      description: "向量嵌入字段，支持语义搜索",
    },
    {
      value: "image",
      label: "图片 (image)",
      description: "图片字段，支持图片检索",
    },
  ];
  ...
  function editField(field: any, index: number) {
    ...
    if (field.embed) {
      ...
      modelConfigItems.value = Object.entries(
        field.embed.model_config || {}
      ).map(([key, value]) => ({
        key,
        value: String(value),
      }));
      // 确保 model_name 存在（编辑时也需要）
      if (!modelConfigItems.value.some((item) => item.key === "model_name")) {
        modelConfigItems.value.push({ key: "model_name", value: "" });
      }
    } else {
      embedFromFields.value = [];
      modelConfigItems.value = [];
  ...
  function saveField(schemaForm: Ref<CollectionCreateSchema>) {
    ...
    if (field.type === "embed") {
      // 校验：至少一个非空的 from
      const validFroms = embedFromFields.value.filter((f) => f && f.trim());
      if (validFroms.length === 0) {
        ElMessage.warning("嵌入字段必须至少包含一个源字段 (from)");
        return;
      }

      // 将 modelConfigItems 转为对象并校验 model_name
      const modelConfigObj = modelConfigItems.value.reduce((acc, item) => {
        acc[item.key] = item.value;
        return acc;
      }, {} as Record<string, string>);

      if (
        !modelConfigObj["model_name"] ||
        !String(modelConfigObj["model_name"]).trim()
      ) {
        ElMessage.warning("模型配置中必须包含非空的 model_name");
        return;
      }

      field.embed = {
        from: [...validFroms],
        model_config: modelConfigObj,
  ...
  function addModelConfigItem() {
    ...
    if (
      modelConfigItems.value.some(
        (item) => item.key === newModelConfigKey.value
      )
    ) {
