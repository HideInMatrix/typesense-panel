...
// // 数据库类型选项
const dbTypes = [{ value: DatabaseType.POSTGRESQL, label: "PostgreSQL" }];
      ...
      <ElDialog
        ...
        <ElForm :model="form" label-width="100px" class="space-y-4">
          ...
          <template
            ...
            <ElFormItem
              v-if="form.type !== DatabaseType.REDIS"
              label="数据库名"
              required>
          ...
          <template
            v-if="
              [DatabaseType.MYSQL, DatabaseType.MARIADB].includes(form.type)
            ">
