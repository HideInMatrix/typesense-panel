...
async function fetchCollections() {
  loading.value = true;
  try {
    const resp = await $fetch("/api/collections/all");
    collections.value = resp?.result || [];
  } catch (error) {
...
async function saveSchema() {
  ...
  try {
    if (editMode.value === "create") {
      const result: { success: boolean; message: string } = (await $fetch(
      ...
      // 编辑模式 - 更新集合
      const result: { success: boolean; message: string } = (await $fetch(
        `/api/collections/${schemaForm.value.name}`,
        {
          method: "PUT",
          body: schemaForm.value,
        }
...
async function loadTableSchema(tableName: string) {
  ...
  try {
    const resp: { data: any[] } = (await $fetch(
      `/api/database/${encodeURIComponent(selectedDb.value)}/${tableName}`
    )) as any;
...
async function applyTableToSchema() {
  ...
  try {
    if (tableColumns.length) {
      ...
      ElMessage.success(
        `已从表 ${selectedTable.value} 导入 ${tableColumns.length} 个字段`
      );
    }
  } catch (e) {
  ...
  <NuxtLayout>
    <div class="space-y-6">
      ...
      <SchemeEditDialog
        ...
        :edit-mode="editMode"
        v-model:schema-form="schemaForm"
        v-model:show-database-import="showDatabaseImport"
        :database-config-list="databaseConfigList"
        ...
        @apply-table="applyTableToSchema"
      ...
      <SchemeFieldDialog
        ...
        v-model:current-field="currentField"
        v-model:embed-from-fields="embedFromFields"
        v-model:model-config-items="modelConfigItems"
        v-model:new-model-config-key="newModelConfigKey"
        v-model:new-model-config-value="newModelConfigValue"
        :field-types="fieldTypes"
        @save="handleSaveField"
        @add-model-config-item="addModelConfigItem"
        @remove-model-config-item="removeModelConfigItem"
        @add-embed-from-field="addEmbedFromField"
