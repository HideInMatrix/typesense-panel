...
async function testConnection() {
  ...
  try {
    ...
    testMsg.value = res.success ? "连接成功" : "连接失败：" + res.message;
  } catch (e: any) {
    testMsg.value = "测试失败";
  } finally {
    testing.value = false;
  ...
  <NuxtLayout>
    <div class="space-y-6">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div
          class="lg:col-span-2 rounded-2xl border border-slate-200/70 bg-white/90 backdrop-blur p-6 shadow-sm">
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <div class="mb-1 text-slate-600 text-sm">Host</div>
              <ElInput
                v-model="(form.nodes as any)[0].host"
                placeholder="typesense.example.com" />
            </div>
            <div>
              <div class="mb-1 text-slate-600 text-sm">Port</div>
              <ElInputNumber
                v-model="(form.nodes as any)[0].port"
                :min="1"
                :max="65535"
                class="w-full" />
            </div>
            <div>
              ...
              <ElSelect
                v-model="(form.nodes as any)[0].protocol"
                class="w-full">
            ...
            <div class="sm:col-span-3">
              ...
              <ElInput
                ...
                type="password"
                show-password
                placeholder="输入 Typesense API Key" />
            </div>
            <div>
              <div class="mb-1 text-slate-600 text-sm">Timeout(s)</div>
              <ElInputNumber
                v-model="form.connectionTimeoutSeconds as number"
                :min="1"
                :max="60"
                class="w-full" />
            </div>
          ...
          <div class="mt-3 space-y-1">
            <div
              class="text-sm"
              :class="
                msg.includes('成功') ? 'text-emerald-600' : 'text-slate-500'
              ">
              {{ msg }}
            </div>
            <div
        ...
        <div
          class="rounded-2xl border border-slate-200/70 bg-white/90 backdrop-blur p-6 shadow-sm">
