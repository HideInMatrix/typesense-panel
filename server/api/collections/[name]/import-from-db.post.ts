...
export default defineEventHandler( async (event) => {
  ...
  if (!name) throw createError({ statusCode: 400, statusMessage: '未指定合集' })

  const body = await readBody(event) as { table: string,database:string }
  const table = body?.table
  const database = body?.database
  if (!table) throw createError({ statusCode: 400, statusMessage: '缺少数据库表' })
  ...
  if (!Array.isArray(rows)) throw createError({ statusCode: 500, statusMessage: '数据结构不对' })

  // 转换为 JSONL
  const jsonl = rows.map((r:any)=> JSON.stringify(r,parseBigInt)).join('\n')
  try{
    const client = await getTypesenseClient()
    const response = await client.collections(name).documents().import(jsonl,{action:'upsert'})
    const result = JSON.parse(response)
    return { success: result.code === 200, ...result}
  }catch(error:any){
